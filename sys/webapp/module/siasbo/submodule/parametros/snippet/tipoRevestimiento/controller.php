...
switch($accion) {
    ...
    default:
        ...
        //
        $grill_list = $subObjItem->get_grilla_list_sbm("grilla_tipoRevestimiento");
    ...
    case 'itemUpdate':
        ...
        $menu_tab = $subObjItem->get_item_tab_sbm($type, "tab_tipoRevestimiento");

        $smarty->assign("menu_tab", $menu_tab);
        $smarty->assign("menu_tab_active", "general");

        if($type=="update"){
            $item = $subObjItem->get_item($id,"catalogo_pozo_perforacion_revestimiento");
    ...
    case 'itemupdatesql':
        //$templateModule = $templateModuleAjax;
        $respuesta = $subObjItem->item_update($item, $itemId, "campos_tipoRevestimiento", $type);
        $core->print_json($respuesta);
        break;
