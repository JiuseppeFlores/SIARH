...
<script>
    ...
    function item_update(id) {
        var url = '{/literal}{$getModule}{literal}&accion=epsas_itemUpdate&id='+id+'&type=update';
        location = url;
    }

    function item_create() {
        var url = '{/literal}{$getModule}{literal}&accion=epsas_itemUpdate&type=new';
        location = url;
    }
    ...
    function itemDeleteAction(id) {
        ...
        $.get( "{/literal}{$getModule}{literal}",
            {accion:"epsas_itemDelete", random:randomnumber, id:id},
            function(res){
                if(res.res == 1){
    ...
    var snippet_datatable_list = function () {
        ...
        var initTable1 = function() {
            ...
            table_list = $('#index_lista').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{/literal}{$getModule}{literal}&accion=epsas_getItemList',
                    type: 'POST',
                    data: {},
                },
                columns: [
