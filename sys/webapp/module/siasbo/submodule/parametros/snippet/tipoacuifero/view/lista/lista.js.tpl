...
<script>
    ...
    function item_update(id) {
        var url = '{/literal}{$getModule}{literal}&accion=tipoacuifero_itemUpdate&id='+id+'&type=update';
        location = url;
    }

    function item_create() {
        var url = '{/literal}{$getModule}{literal}&accion=tipoacuifero_itemUpdate&type=new';
        location = url;
    }
    ...
    function itemDeleteAction(id) {
        ...
        $.get( "{/literal}{$getModule}{literal}",
            {accion:"tipoacuifero_itemDelete", random:randomnumber, id:id},
            function(res){
                if(res.res == 1){
    ...
    var snippet_datatable_list = function () {
        ...
        var initTable1 = function() {
            ...
            table_list = $('#index_lista').DataTable({
                ...
                pageLength: 10,
                order: [[ 1, "asc" ]], // Por que campo ordenara al momento de despltipoacuifero
                searchDelay: 500,
                processing: true,
                serverSide: true,
                ajax: {
                    url: '{/literal}{$getModule}{literal}&accion=tipoacuifero_getItemList',
                    type: 'POST',
                    data: {},
                },
                columns: [
