...
switch($accion) {
    ...
    default:
        ...
        //
        $grill_list = $subObjItem->get_grilla_list_sbm("grilla_tipoRejillaFiltro");
    ...
    case 'itemUpdate':
        ...
        $menu_tab = $subObjItem->get_item_tab_sbm($type, "tab_tipoRejillaFiltro");

        $smarty->assign("menu_tab", $menu_tab);
        $smarty->assign("menu_tab_active", "general");

        if($type=="update"){
            $item = $subObjItem->get_item($id,"catalogo_pozo_constructivo_rejillafiltro");
    ...
    case 'itemupdatesql':
        //$templateModule = $templateModuleAjax;
        $respuesta = $subObjItem->item_update($item, $itemId, "campos_tipoRejillaFiltro", $type);
        $core->print_json($respuesta);
        break;
