...
switch($accion) {
    ...
    default:
        ...
        //
        $grill_list = $subObjItem->get_grilla_list_sbm("grilla_tipoParametroCalidad");
    ...
    case 'itemUpdate':
        ...
        $menu_tab = $subObjItem->get_item_tab_sbm($type, "tab_tipoParametroCalidad");

        $smarty->assign("menu_tab", $menu_tab);
        $smarty->assign("menu_tab_active", "general");

        if($type=="update"){
            $item = $subObjItem->get_item($id,"catalogo_pozo_monitor_calidad_parametro");
    ...
    case 'itemupdatesql':
        //$templateModule = $templateModuleAjax;
        $respuesta = $subObjItem->item_update($item, $itemId, "campos_tipoParametroCalidad", $type);
        $core->print_json($respuesta);
        break;
