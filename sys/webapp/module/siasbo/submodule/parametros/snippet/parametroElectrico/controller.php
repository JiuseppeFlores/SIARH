...
switch($accion) {
    ...
    default:
        ...
        //
        $grill_list = $subObjItem->get_grilla_list_sbm("grilla_parametroElectrico");
    ...
    case 'itemUpdate':
        ...
        $menu_tab = $subObjItem->get_item_tab_sbm($type, "tab_parametroElectrico");

        $smarty->assign("menu_tab", $menu_tab);
        $smarty->assign("menu_tab_active", "general");

        if($type=="update"){
            $item = $subObjItem->get_item($id,"catalogo_pozo_electrico_parametro");
    ...
    case 'itemupdatesql':
        //$templateModule = $templateModuleAjax;
        $respuesta = $subObjItem->item_update($item, $itemId, "campos_parametroElectrico", $type);
        $core->print_json($respuesta);
        break;
