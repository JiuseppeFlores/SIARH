...
class Snippet extends Table {
    ...
    function get_ficha($id) {
        ...
        $sql = "SELECT c.nombre AS acuifero, 
        b.nombre AS fuente, 
        b.itemId AS fuente_id, 
        count(*) AS total 
        FROM item a, catalogo_tipo b, catalogo_acuifero c  
        WHERE a.tipo=b.itemId 
        AND a.acuiferoId=c.itemId 
        AND a.acuiferoId=".(int) $id."
        GROUP BY a.tipo, a.acuiferoId 
        ORDER BY total DESC";
    ...
    function get_ficha_campania($id, $mes, $anio) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT c.nombre AS acuifero, 
        b.nombre AS fuente, 
        b.itemId AS fuente_id, 
        count(*) AS total 
        FROM item a, catalogo_tipo b, catalogo_acuifero c, item_pozo_monitor_calidad d
        WHERE a.tipo=b.itemId 
        AND a.acuiferoId=c.itemId 
        AND a.itemId=d.pozoId
        AND a.acuiferoId=".(int) $id."
        AND DATE_FORMAT(d.fecha_muestreo, '%m')=".(int) $mes."
        AND DATE_FORMAT(d.fecha_muestreo, '%Y')=".(int) $anio."
        GROUP BY a.tipo, a.acuiferoId 
        ORDER BY total DESC";
        $datos = $this->dbm->Execute($sql);
        $datos = $datos->GetRows();
    ...
    function get_ficha_epsas_campania($id, $mes, $anio) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT b.nombre AS epsa, 
        count(*) AS total 
        FROM item a, catalogo_pozo_general_epsas b, item_pozo_monitor_calidad c 
        WHERE a.epsasId=b.itemId 
        AND a.itemId=c.pozoId
        AND a.tipo=1 
        AND a.acuiferoId=".(int) $id."
        AND DATE_FORMAT(c.fecha_muestreo, '%m')=".(int) $mes."
        AND DATE_FORMAT(c.fecha_muestreo, '%Y')=".(int) $anio."
        GROUP BY a.epsasId 
        ORDER BY total DESC";
        $datos = $this->dbm->Execute($sql);
        $datos = $datos->GetRows();
    ...
    function get_ficha_anio_perforacion($acuiferoId, $anio) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT c.nombre AS acuifero, 
        b.nombre AS fuente, 
        b.itemId AS fuente_id, 
        count(*) AS total 
        FROM item a, catalogo_tipo b, catalogo_acuifero c, item_pozo d
        WHERE a.tipo=b.itemId 
        AND a.acuiferoId=c.itemId 
    ...
    function get_ficha_epsas_anio_perforacion($acuiferoId, $anio) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT b.nombre AS epsa, 
        count(*) AS total 
        FROM item a, catalogo_pozo_general_epsas b, item_pozo c 
        WHERE a.epsasId=b.itemId 
    ...
    function get_ficha_profundidad_menor($acuiferoId, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT c.nombre AS acuifero, 
        b.nombre AS fuente, 
        b.itemId AS fuente_id, 
    ...
    function get_ficha_epsas_profundidad_menor($acuiferoId, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT b.nombre AS epsa, 
        count(*) AS total 
        FROM item a, catalogo_pozo_general_epsas b, item_pozo c 
    ...
    function get_ficha_profundidad_mayor($acuiferoId, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT c.nombre AS acuifero, 
        b.nombre AS fuente, 
        b.itemId AS fuente_id, 
    ...
    function get_ficha_epsas_profundidad_mayor($acuiferoId, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT b.nombre AS epsa, 
        count(*) AS total 
        FROM item a, catalogo_pozo_general_epsas b, item_pozo c 
    ...
    function get_ficha_profundidad($acuiferoId, $desde, $hasta) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT c.nombre AS acuifero, 
        b.nombre AS fuente, 
        b.itemId AS fuente_id, 
    ...
    function get_ficha_epsas_profundidad($acuiferoId, $desde, $hasta) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT b.nombre AS epsa, 
        count(*) AS total 
        FROM item a, catalogo_pozo_general_epsas b, item_pozo c 
    ...
    function get_ficha_uso($acuiferoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT t1.nombre, t2.acuifero, t2.total
        FROM 
        (SELECT itemId, nombre 
        FROM catalogo_pozo_usoagua) t1 
        LEFT JOIN 
        (SELECT c.nombre AS acuifero, 
        e.itemId, 
        count(*) AS total 
        FROM item a, catalogo_tipo b, catalogo_acuifero c, item_pozo d, catalogo_pozo_usoagua e 
        WHERE a.tipo=b.itemId 
        AND a.acuiferoId=c.itemId 
        AND a.itemId=d.itemId 
        AND a.tipo=1 
    ...
    function get_ficha_proposito($acuiferoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT t1.nombre, t2.acuifero, t2.total
        FROM 
        (SELECT itemId, nombre 
        ...
        e.itemId, 
        count(*) AS total 
        FROM item a, catalogo_tipo b, catalogo_acuifero c, item_pozo d, catalogo_pozo_proposito e 
        WHERE a.tipo=b.itemId 
        AND a.acuiferoId=c.itemId 
        AND a.itemId=d.itemId 
        AND a.tipo=1 
        ...
        GROUP BY a.acuiferoId 
        ORDER BY total DESC, e.nombre ASC) t2
        ON t1.itemId=t2.itemId 
        ORDER BY t2.total DESC, t1.nombre ASC";
        $datos = $this->dbm->Execute($sql);
        $datos = $datos->GetRows();
    ...
    function get_puntos_acuifero($tipo, $acuiferoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_coordenada_min_max_acuifero($acuiferoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
    ...
    function get_coordenada_min_max_acuifero_pozos($acuiferoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT MIN(longitudDec) AS longitudMin, 
        MIN(latitudDec) AS latitudMin, 
        MAX(longitudDec) AS longitudMax, 
    ...
    function get_puntos_campania($tipo, $acuiferoId, $mes, $anio) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
        ...
        a.longitudDec, 
        a.latitudDec 
        FROM item a, catalogo_tipo b, item_pozo_monitor_calidad c 
        WHERE a.tipo=b.itemId 
        AND a.itemId=c.pozoId 
        AND (a.longitudDec BETWEEN -70 AND -55) 
        AND (a.latitudDec BETWEEN -24 AND -8) 
    ...
    function get_coordenada_min_max_campania($acuiferoId, $mes, $anio) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT MIN(a.longitudDec) AS longitudMin, 
        MIN(a.latitudDec) AS latitudMin, 
        MAX(a.longitudDec) AS longitudMax, 
        MAX(a.latitudDec) AS latitudMax 
        FROM item a, item_pozo_monitor_calidad b 
        WHERE a.itemId=b.pozoId 
        AND (a.longitudDec BETWEEN -70 AND -55) 
        AND (a.latitudDec BETWEEN -24 AND -8) 
        AND a.acuiferoId=".$acuiferoId."
        AND DATE_FORMAT(b.fecha_muestreo, '%m')=".(int) $mes."
        AND DATE_FORMAT(b.fecha_muestreo, '%Y')=".(int) $anio."
        LIMIT 1";
        $datos = $this->dbm->Execute($sql);
        $datos = $datos->GetRows();
        return $datos;
    ...
    function get_puntos_anio_perforacion($acuiferoId, $anio) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_puntos_profundidad_menor($acuiferoId, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_puntos_profundidad_mayor($acuiferoId, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_puntos_profundidad($acuiferoId, $desde, $hasta) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_puntos_uso($acuiferoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_puntos_proposito($acuiferoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_campanias($acuiferoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT
        DATE_FORMAT(b.fecha_muestreo, '%m') AS mes,
        DATE_FORMAT(b.fecha_muestreo, '%Y') AS anio
        FROM item a, item_pozo_monitor_calidad b
        WHERE a.itemId=b.pozoId
        AND a.acuiferoId=".$acuiferoId."
        GROUP BY mes, anio
    ...
    function get_anio_perforacion($acuiferoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT
        DATE_FORMAT(b.perforacion_fecha, '%Y') AS anio
        FROM item a, item_pozo b
        WHERE a.itemId=b.itemId
        AND a.tipo=1
        AND a.acuiferoId=".$acuiferoId."
        AND DATE_FORMAT(b.perforacion_fecha, '%Y') <> ''
        GROUP BY anio
        ORDER BY anio DESC";
        $datos = $this->dbm->Execute($sql);
        $datos = $datos->GetRows();
    ...
    function get_compuestos($acuiferoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT e.itemId, 
        e.nombre AS acuifero, 
        d.itemId AS pozoId, 
        d.codigo, 
        d.nombre, 
    ...
    function get_compuestos_campania($acuiferoId, $mes, $anio) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT e.itemId, 
        e.nombre AS acuifero, 
        d.itemId AS pozoId, 
        ...
        item d,
        catalogo_acuifero e
        WHERE a.itemId=b.compuestoId 
        AND b.calidadId=c.itemId 
        AND c.pozoId=d.itemId 
        AND d.acuiferoId=e.itemId 
        AND b.compuestoId IN (54,75,90,93,59,47,94) 
        AND e.itemId=".$acuiferoId." 
        AND DATE_FORMAT(c.fecha_muestreo, '%m')=".(int) $mes." 
        AND DATE_FORMAT(c.fecha_muestreo, '%Y')=".(int) $anio." 
        GROUP BY b.calidadId 
        ORDER BY c.pozoId, b.calidadId ASC";
    ...
    function get_reporte_calidad($acuiferoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT
        a.itemId, a.codigo, a.nombre, c.compuestoId, c.valor  
        FROM item a, 
        item_pozo_monitor_calidad b, 
        item_pozo_monitor_calidad_dato c  
        WHERE a.itemId=b.pozoId 
        AND b.itemId=c.calidadId 
        AND c.compuestoId IN (8,27,37,54,59,70,71,75,83,93,94) 
        AND a.acuiferoId=".$acuiferoId." 
        ORDER BY a.itemId ASC";
        $datos = $this->dbm->Execute($sql);
        $datos = $datos->GetRows();
    ...
    function get_reporte_calidad_campania($acuiferoId, $mes, $anio) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT
        a.itemId, a.codigo, a.nombre, c.compuestoId, c.valor  
        FROM item a, 
    ...
    /*function get_datos_pozos($acuiferoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT t1.*, t2.*
        FROM 
        ...
        AND a.tipo=1 
        AND a.acuiferoId=".$acuiferoId.") t1 
        LEFT JOIN 
        ...
        WHERE a.itemId=b.itemId 
        AND a.tipo=1 
        AND a.acuiferoId=".$acuiferoId.") t2 
        ON t1.itemId=t2.id
    ...
    function get_datos_pozos_excel($metadatos, $tituloDocumento, $acuiferoId) {
        ...
        $respuesta = $this->get_datos_pozos($acuiferoId);
        ...
        $this->descargar_documento_excel($documentoHojaCalculo, 'pozos_acuifero');
    }

    function get_archivo_csv($tituloDocumento, $nombreArchivo, $acuiferoId) {
    ...
    function get_campanias_isotopico($acuiferoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
    ...
    function get_isotopicos($acuiferoId, $mes, $anio){
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT a.nombre, b.nombre AS pozo, GROUP_CONCAT(d.valor ORDER BY d.valor) AS valor 
        FROM catalogo_acuifero a, item b, item_pozo_monitor_isotopico c, item_pozo_monitor_isotopico_dato d 
        WHERE a.itemId=b.acuiferoId AND b.itemId=c.pozoId AND c.itemId=d.isotopicoId 
        AND d.isotocompuestoId IN (10, 12)
        AND DATE_FORMAT(c.fecha_muestreo, '%m')=".$mes." 
        AND DATE_FORMAT(c.fecha_muestreo, '%Y')=".$anio."
        AND a.itemId=".$acuiferoId."
