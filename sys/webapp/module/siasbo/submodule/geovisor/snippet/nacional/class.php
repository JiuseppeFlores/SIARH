...
class Snippet extends Table {
    ...
    function get_ficha_anio_perforacion($anio) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT c.nombre AS departamento, 
        b.nombre AS fuente, 
        b.itemId AS fuente_id, 
    ...
    function get_ficha_epsas_anio_perforacion($anio) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT b.nombre AS epsa, 
        count(*) AS total 
    ...
    function get_ficha_profundidad_menor($tipo, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT c.nombre AS departamento, 
        b.nombre AS fuente, 
        b.itemId AS fuente_id, 
    ...
    function get_ficha_profundidad_mayor($tipo, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT c.nombre AS departamento, 
        b.nombre AS fuente, 
        b.itemId AS fuente_id, 
    ...
    function get_ficha_profundidad($tipo, $desde, $hasta) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT c.nombre AS departamento, 
        b.nombre AS fuente, 
        b.itemId AS fuente_id, 
    ...
    function get_ficha_epsas_profundidad_menor($tipo, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT b.nombre AS epsa, 
        count(*) AS total 
        FROM item a, catalogo_pozo_general_epsas b, item_pozo c 
    ...
    function get_ficha_epsas_profundidad_mayor($tipo, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT b.nombre AS epsa, 
        count(*) AS total 
        FROM item a, catalogo_pozo_general_epsas b, item_pozo c 
    ...
    function get_ficha_epsas_profundidad($tipo, $desde, $hasta) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT b.nombre AS epsa, 
        count(*) AS total 
        FROM item a, catalogo_pozo_general_epsas b, item_pozo c 
    ...
    function get_ficha_uso() {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT t1.nombre, t2.total
        FROM 
        (SELECT itemId, nombre 
        FROM catalogo_pozo_usoagua) t1 
        LEFT JOIN 
        (SELECT a.itemId, COUNT(*) AS total
        FROM catalogo_pozo_usoagua a, 
        item_pozo b, 
        item c
    ...
    /*function get_ficha_uso() {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT a.itemId, a.nombre, COUNT(*) AS total
        FROM catalogo_pozo_usoagua a, 
        item_pozo b, 
        item c
        WHERE b.itemId=c.itemId
    ...
    function get_ficha_proposito() {
        ...
        LEFT JOIN 
        (SELECT a.itemId, COUNT(*) AS total
        FROM catalogo_pozo_proposito a, 
        item_pozo b, 
        item c
        WHERE b.itemId=c.itemId
        AND c.tipo=1
    ...
    function get_puntos_anio_perforacion($tipo, $anio) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_puntos_profundidad_menor($tipo, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_puntos_profundidad_mayor($tipo, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_puntos_profundidad($tipo, $desde, $hasta) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_cantidad_datos_pozos() {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
    ...
    function descargar_documento_excel($documentoHojaCalculo, $nombreArchivo) {
        ...
        // Memory cleanup.
        $documentoHojaCalculo->disconnectWorksheets();
        unset($documentoHojaCalculo);
    }

    function get_archivo_excel($metadatos, $tituloDocumento) {
        $nombreArchivo = "pozos_nacional.xlsx";
        ...
        $respuesta = $this->get_datos_pozos();
        ...
        foreach ($respuesta as $clave => $valor) {
            ...
            $hoja->setCellValue('R'.$contador, $catalogo_acuiferos[$valor['acuiferoId']]);
            ...
            $hoja->setCellValue('W'.$contador, $catalogo_cuencas_estrategicas[$valor['cuencaestrategicaId']]);
            // Datos específicos
        ...
        $documentoHojaCalculo->setActiveSheetIndex(0);
        ...
        $writer = IOFactory::createWriter($documentoHojaCalculo, 'Xlsx');
        $writer->save('php://output');
        exit();*/
        ...
        // If you're serving to IE 9, then the following may be needed
        header('Cache-Control: max-age=1');
        $writer = \PhpOffice\PhpSpreadsheet\IOFactory::createWriter($documentoHojaCalculo, 'Xlsx');
        ob_end_clean();
        $writer->save('php://output');
        exit();
    }

    function get_archivo_csv($tituloDocumento, $nombreArchivo) {
