...
class Snippet extends Table {
    ...
    function get_ficha($cuencaestrategicaId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT e.nombre AS cuenca_estrategica, 
        d.nombre AS municipio, 
        b.nombre AS fuente, b.itemId AS fuente_id, 
        count(*) AS total 
        FROM item a, catalogo_tipo b, 
        vrhr_territorio.municipio d, 
        catalogo_cuenca_estrategica e
        WHERE a.tipo=b.itemId 
        AND a.municipioId=d.itemId 
        AND a.cuencaestrategicaId=e.itemId
        AND a.cuencaestrategicaId=".(int) $cuencaestrategicaId."
        GROUP BY a.tipo, a.municipioId 
        ORDER BY total DESC";
    ...
    function get_ficha_epsas($cuencaestrategicaId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
    ...
    function get_ficha_anio_perforacion($cuencaestrategicaId, $anio) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT d.nombre AS cuenca_estrategica, 
        c.nombre AS municipio, 
        b.nombre AS fuente, b.itemId AS fuente_id, 
        count(*) AS total 
        FROM item a, catalogo_tipo b, 
        vrhr_territorio.municipio c, 
        catalogo_cuenca_estrategica d, 
        item_pozo e 
        WHERE a.tipo=b.itemId 
        AND a.municipioId=c.itemId 
        AND a.cuencaestrategicaId=d.itemId 
        AND a.itemId=e.itemId 
        AND a.tipo=1 
        AND a.cuencaestrategicaId=".$cuencaestrategicaId."
        AND DATE_FORMAT(e.perforacion_fecha, '%Y')=".$anio."
        GROUP BY a.municipioId 
        ORDER BY total DESC";
        $datos = $this->dbm->Execute($sql);
        $datos = $datos->GetRows();
        return $datos;
    ...
    function get_ficha_epsas_anio_perforacion($cuencaestrategicaId, $anio) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT b.nombre AS epsa, 
        count(*) AS total 
    ...
    function get_ficha_profundidad_menor($cuencaestrategicaId, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT d.nombre AS cuenca_estrategica, 
        c.nombre AS municipio, 
        b.nombre AS fuente, b.itemId AS fuente_id, 
        ...
        AND a.tipo=1 
        AND a.cuencaestrategicaId=".$cuencaestrategicaId." 
        AND e.perforacion_profundidad < ".$valor." 
        GROUP BY a.municipioId 
        ORDER BY total DESC";
        $datos = $this->dbm->Execute($sql);
        $datos = $datos->GetRows();
    ...
    function get_ficha_epsas_profundidad_menor($cuencaestrategicaId, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT b.nombre AS epsa, 
        count(*) AS total 
        FROM item a, catalogo_pozo_general_epsas b, item_pozo c 
    ...
    function get_ficha_profundidad_mayor($cuencaestrategicaId, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT d.nombre AS cuenca_estrategica, 
        c.nombre AS municipio, 
        b.nombre AS fuente, b.itemId AS fuente_id, 
    ...
    function get_ficha_epsas_profundidad_mayor($cuencaestrategicaId, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT b.nombre AS epsa, 
        count(*) AS total 
        FROM item a, catalogo_pozo_general_epsas b, item_pozo c 
    ...
    function get_ficha_profundidad($cuencaestrategicaId, $desde, $hasta) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT d.nombre AS cuenca_estrategica, 
        c.nombre AS municipio, 
        b.nombre AS fuente, b.itemId AS fuente_id, 
        ...
        AND a.tipo=1 
        AND a.cuencaestrategicaId=".$cuencaestrategicaId." 
        AND e.perforacion_profundidad >= ".$desde." 
        AND e.perforacion_profundidad <= ".$hasta." 
        GROUP BY a.municipioId 
        ORDER BY total DESC";
        $datos = $this->dbm->Execute($sql);
        $datos = $datos->GetRows();
    ...
    function get_ficha_epsas_profundidad($cuencaestrategicaId, $desde, $hasta) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT b.nombre AS epsa, 
        count(*) AS total 
        FROM item a, catalogo_pozo_general_epsas b, item_pozo c 
    ...
    function get_ficha_uso($cuencaestrategicaId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT t1.nombre, t2.cuenca_estrategica, t2.total
        FROM 
        (SELECT itemId, nombre AS nombre 
        FROM catalogo_pozo_usoagua) t1 
        LEFT JOIN 
        (SELECT c.nombre AS cuenca_estrategica, 
        e.itemId, 
        count(*) AS total 
        FROM item a, catalogo_tipo b, catalogo_cuenca_estrategica c, item_pozo d, catalogo_pozo_usoagua e 
        WHERE a.tipo=b.itemId 
        AND a.cuencaestrategicaId=c.itemId 
        AND a.itemId=d.itemId 
        AND a.tipo=1 
        AND a.cuencaestrategicaId=".$cuencaestrategicaId."
        AND FIND_IN_SET(e.itemId, d.usoaguaId) 
        GROUP BY a.cuencaestrategicaId) t2
    ...
    /*function get_ficha_uso($cuencaestrategicaId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT c.nombre AS cuenca_estrategica, 
        e.nombre AS nombre, 
        count(*) AS total 
        FROM item a, catalogo_tipo b, catalogo_cuenca_estrategica c, item_pozo d, catalogo_pozo_usoagua e 
        WHERE a.tipo=b.itemId 
        ...
        AND a.cuencaestrategicaId=".$cuencaestrategicaId."
        AND FIND_IN_SET(e.itemId, d.usoaguaId) 
        GROUP BY a.cuencaestrategicaId 
    ...
    function get_ficha_proposito($cuencaestrategicaId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT t1.nombre, t2.cuenca_estrategica, t2.total
        FROM 
        (SELECT itemId, nombre AS nombre 
        ...
        e.itemId, 
        count(*) AS total 
        FROM item a, catalogo_tipo b, catalogo_cuenca_estrategica c, item_pozo d, catalogo_pozo_proposito e 
        WHERE a.tipo=b.itemId 
        AND a.cuencaestrategicaId=c.itemId 
        AND a.itemId=d.itemId 
        AND a.tipo=1 
    ...
    function get_puntos($tipo, $cuencaestrategicaId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_puntos_anio_perforacion($cuencaestrategicaId, $anio) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_puntos_profundidad_menor($cuencaestrategicaId, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_puntos_profundidad_mayor($cuencaestrategicaId, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_puntos_profundidad($cuencaestrategicaId, $desde, $hasta) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_puntos_uso($cuencaestrategicaId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_puntos_proposito($cuencaestrategicaId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_coordenada_min_max($cuencaestrategicaId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        ...
        AND (latitudDec BETWEEN -24 AND -8) 
        AND cuencaestrategicaId=".$cuencaestrategicaId." 
        LIMIT 1";
    ...
    function get_puntos_pg($tipo, $cuencaestrategicaId) {
        ...
        FROM item 
        WHERE tipo=".$tipo." 
        AND (CAST(longitudDec AS DOUBLE PRECISION) BETWEEN -70 AND -55) 
        AND (CAST(latitudDec AS DOUBLE PRECISION) BETWEEN -24 AND -8) 
        AND ST_Contains((SELECT geom FROM cuenca_estra_25 WHERE id=".$cuencaestrategicaId."), ST_GeomFromText(CONCAT('POINT(',longitudDec, ' ', latitudDec, ')'),4326))";
    ...
    function get_anio_perforacion($cuencaestrategicaId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT
    ...
    /*function get_datos_pozos($cuencaestrategicaId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT t1.*, t2.*
        FROM 
        ...
        AND a.tipo=1 
        AND a.cuencaestrategicaId=".$cuencaestrategicaId.") t1 
        LEFT JOIN 
        ...
        WHERE a.itemId=b.itemId 
        AND a.tipo=1 
        AND a.cuencaestrategicaId=".$cuencaestrategicaId.") t2 
        ON t1.itemId=t2.id
    ...
    function get_datos_pozos_excel($metadatos, $tituloDocumento, $cuencaestrategicaId) {
        ...
        $respuesta = $this->get_datos_pozos($cuencaestrategicaId);
        ...
        $this->descargar_documento_excel($documentoHojaCalculo, 'pozos_cuenca_estrategica');
    }

    function get_archivo_csv($tituloDocumento, $nombreArchivo, $cuencaestrategicaId) {
