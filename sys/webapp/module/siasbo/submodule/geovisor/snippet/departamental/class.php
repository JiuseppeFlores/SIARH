...
class Snippet extends Table {
    ...
    function get_ficha_epsas($deptoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
    ...
    function get_ficha_anio_perforacion($deptoId, $anio) {
        $sql = "SELECT c.nombre AS departamento, 
        d.nombre AS municipio, 
        b.nombre AS fuente, 
        b.itemId AS fuente_id, 
    ...
    function get_ficha_epsas_anio_perforacion($deptoId, $anio) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT b.nombre AS epsa, 
        count(*) AS total 
    ...
    function get_ficha_profundidad_menor($tipo, $deptoId, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT c.nombre AS departamento, 
        b.nombre AS fuente, 
        b.itemId AS fuente_id, 
        count(*) AS total 
        FROM item a, catalogo_tipo b, 
        vrhr_territorio.departamento c, item_pozo d 
        WHERE a.tipo=b.itemId 
        AND a.departamentoId=c.itemId 
    ...
    function get_ficha_profundidad_mayor($tipo, $deptoId, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT c.nombre AS departamento, 
        b.nombre AS fuente, 
        b.itemId AS fuente_id, 
        ...
        AND d.perforacion_profundidad > ".$valor." 
        AND a.departamentoId=".$deptoId." 
        GROUP BY a.tipo, a.departamentoId 
        ORDER BY total DESC";
        $datos = $this->dbm->Execute($sql);
        $datos = $datos->GetRows();
        return $datos;
    ...
    function get_ficha_profundidad($tipo, $deptoId, $desde, $hasta) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT c.nombre AS departamento, 
        b.nombre AS fuente, 
        b.itemId AS fuente_id, 
    ...
    function get_ficha_epsas_profundidad_menor($tipo, $deptoId, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT b.nombre AS epsa, 
        count(*) AS total 
        FROM item a, catalogo_pozo_general_epsas b, item_pozo c 
    ...
    function get_ficha_epsas_profundidad_mayor($tipo, $deptoId, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT b.nombre AS epsa, 
        count(*) AS total 
        FROM item a, catalogo_pozo_general_epsas b, item_pozo c 
    ...
    function get_ficha_epsas_profundidad($tipo, $deptoId, $desde, $hasta) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT b.nombre AS epsa, 
        count(*) AS total 
        FROM item a, catalogo_pozo_general_epsas b, item_pozo c 
    ...
    function get_ficha_uso($deptoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT t1.nombre, t2.departamento, t2.total
        FROM 
        ...
        a.itemId,  
        COUNT(*) AS total
        FROM catalogo_pozo_usoagua a, item_pozo b, 
        item c, vrhr_territorio.departamento d
        WHERE b.itemId=c.itemId
        AND c.departamentoId=d.itemId
        AND c.tipo=1 
        AND c.departamentoId=".$deptoId." 
        AND FIND_IN_SET(a.itemId, b.usoaguaId) 
        GROUP BY a.itemId) t2
    ...
    function get_ficha_proposito($deptoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT t1.nombre, t2.departamento, t2.total
        FROM 
        ...
        a.itemId, 
        COUNT(*) AS total
        FROM catalogo_pozo_proposito a, item_pozo b, 
        item c, vrhr_territorio.departamento d
        WHERE b.itemId=c.itemId
        AND c.departamentoId=d.itemId
        AND c.tipo=1 
        AND c.departamentoId=".$deptoId." 
        AND FIND_IN_SET(a.itemId, b.propositoId) 
        GROUP BY a.itemId) t2
        ON t1.itemId=t2.itemId 
        ORDER BY t2.total DESC, t1.nombre ASC";
        $datos = $this->dbm->Execute($sql);
    ...
    function get_puntos($tipo, $deptoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_puntos_anio_perforacion($tipo, $deptoId, $anio) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_puntos_profundidad_menor($tipo, $deptoId, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
        ...
        AND (latitudDec BETWEEN -24 AND -8) 
        AND a.tipo=".$tipo." 
        AND c.perforacion_profundidad > 0
        AND c.perforacion_profundidad < ".$valor." 
        AND a.departamentoId=".$deptoId." 
        ORDER BY a.itemId ASC";
        $datos = $this->dbm->Execute($sql);
    ...
    function get_puntos_profundidad_mayor($tipo, $deptoId, $valor) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_puntos_profundidad($tipo, $deptoId, $desde, $hasta) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_puntos_uso($deptoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_puntos_proposito($deptoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
         $sql = "SELECT a.itemId, 
        a.codigo, 
        a.nombre, 
    ...
    function get_coordenada_min_max($deptoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
    ...
    function get_fuentes_pg($tipo, $deptoId) {
        $dbconn = pg_connect("host=localhost port=5432 dbname=bolivia user=postgres password=admin");
        $sql = "SELECT itemId, 
        codigo, 
        nombre, 
        tipo, 
        ...
        AND ST_Contains((SELECT geom FROM departamento WHERE id=".$deptoId."), ST_GeomFromText(CONCAT('POINT(',longitudDec, ' ', latitudDec, ')'),4326))
        ";
        ...
        while ($row = pg_fetch_row($result)) {
            $datos[] = array('itemId' => $row[0], 'codigo' => $row[1], 'nombre' => $row[2], 'tipo' => $row[3], 'tipoNombre' => $row[4], 'longitudDec' => $row[5], 'latitudDec' => $row[6]);
        }
    ...
    function get_anio_perforacion($deptoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT
        DATE_FORMAT(b.perforacion_fecha, '%Y') AS anio
        FROM item a, item_pozo b
    ...
    /*function get_datos_pozos($deptoId) {
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT t1.*, t2.*
        FROM 
        ...
        AND a.tipo=1 
        AND a.departamentoId=".$deptoId.") t1 
        LEFT JOIN 
        ...
        WHERE a.itemId=b.itemId 
        AND a.tipo=1 
        AND a.departamentoId=".$deptoId.") t2 
        ON t1.itemId=t2.id
    ...
    function get_datos_pozos_excel($metadatos, $tituloDocumento, $deptoId) {
        ...
        $respuesta = $this->get_datos_pozos($deptoId);
        ...
        $this->descargar_documento_excel($documentoHojaCalculo, 'pozos_departamental');
    }

    function get_archivo_csv($tituloDocumento, $nombreArchivo, $deptoId) {
