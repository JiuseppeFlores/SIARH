...
//----------------Permisos--------------------------------------------
    ...
    function GuardarGraficaCalidadS(){
        Canvas2Image.saveAsPNG(canvasstiff, canvasstiff.width, canvasstiff.height, "DiagramaStiffManantial");
    }

    function AbrirModalCalidadS(){
        $('#modalgraficastiffCalidad').modal('show');
    }

    function CerrarModalCalidadS(){
        swal({type: 'success', title: 'Cerrando!', showConfirmButton: false, timer: 300});
        $('#modalgraficastiffCalidad').modal('hide');

    }

    var select_campania;
    var canvasstiff = document.getElementById("lienzo_stiff");    
    ...
    function RecuperarDatosStiff(campania){
        ...
        $.get(url, function(respuesta) {
            ...
            console.log("Datos stiff manantial calidad: ", datos);            
            dtstiff = datos;
            CargarGraficaCalidadS();
            //alert("Nivel estatico"+datos[0].nivel_estatico);
        });
    }


    function CargarGraficaCalidadS(){
        Datos();
    ...
    jQuery(document).ready(function() {
        ...
        $('.select2').select2({
            ...
            dropdownParent: $("#modalgraficastiffCalidad"),
        });
