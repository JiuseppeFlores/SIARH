{literal}
<script>
    var table_list_tomografia_vinculo_pozo;

    function item_update_tomografia_vinculo_pozo(id) {
        //var url = '{/literal}{$getModule}{literal}&accion={/literal}{$subcontrol}_itemUpdateCapa{literal}&id='+id+'&type=update';
        //location = url;
        get_form_tomografia_vinculo_pozo(id, "update");
    }

    function item_delete_tomografia_vinculo_pozo(id) {
        swal({
            ...
            if (result.value) {
                itemDeleteActionTomografiaVinculoPozo(id);
            }
        });
    }

    function item_print_tomografia_vinculo_pozo(id) {
        alert("Imprime el dato:"+id);
    }

    function itemDeleteActionTomografiaVinculoPozo(id) {
    ...
    function item_opcion_editar_tomografia_vinculo_pozo(id) {
        return '<a class="dropdown-item" href="javascript:item_update_tomografia_vinculo_pozo(\''+ id +'\');"><i class="la flaticon-edit-1 m--font-brand"></i>&nbsp;Editar</a>';
    }

    function item_opcion_eliminar_tomografia_vinculo_pozo(id) {
        return '<a class="dropdown-item" href="javascript:item_delete_tomografia_vinculo_pozo(\''+ id +'\');"><i class="la flaticon-delete-2 m--font-brand"></i>&nbsp;Eliminar</a>';
    }

    function get_form_tomografia_vinculo_pozo(id, type) {
        ...
        $.get(url, function(respuesta) {
            $('#modal_content_form_tomografia_vinculo_pozo').html(respuesta);
            show_modal_window_form_tomografia_vinculo_pozo();
        });
    }
    
    var snippet_datatable_list_tomografia_vinculo_pozo = function () {
        ...
        var initTable = function() {
            // begin first table
            table_list_tomografia_vinculo_pozo = $('#lista_tomografia_vinculo_pozo').DataTable({
                responsive: true,
                //== Pagination settings
                dom:
                ...
                columnDefs: [
                    {
                        ...
                        render: function(data, type, full, meta) {
                            ...
                            if (obj_permiso[0].editar == 1){ //permisoedit
                                botonedit = '{/literal}{if $privFace.editar == 1}{literal}' + item_opcion_editar_tomografia_vinculo_pozo(data) + '{/literal}{/if}{literal}';
                            }

                            if (obj_permiso[0].eliminar == 1){ //permisodelete
                                botondelete = '{/literal}{if $privFace.eliminar == 1}{literal}' + item_opcion_eliminar_tomografia_vinculo_pozo(data) + '{/literal}{/if}{literal}';
                            }
...
//----------------Permisos--------------------------------------------
    ...
    jQuery(document).ready(function() {
        permisos_usuario();
        snippet_datatable_list_tomografia_vinculo_pozo.init();
    });
