{literal}
<script>
    var snippet_form_sev = function () {
        var idficha = '{/literal}{$id}{literal}';
        var type = '{/literal}{$type}{literal}';
        var form_sev = $('#form_sev');
        var btn_sev_submit = $('#btn_sev_submit');

        var campo_input_sev = $('#form_sev input');

        //== Private Functions
        var showRequest= function (formData, jqForm, op) {
            btn_sev_submit.addClass('m-loader m-loader--right m-loader--light').attr('disabled', true);
            return true;
        };

        var showResponse = function (responseText, statusText) {
            btn_sev_submit.removeClass('m-loader m-loader--right m-loader--light').attr('disabled', false);
        ...
        var handle_form_submit = function () {
            form_sev.ajaxForm(options);
        };

        var handle_general_form_submit = function () {

            btn_sev_submit.click(function (e) {
                e.preventDefault();
                var btn = $(this);
        ...
        var handle_get_components = function () {
            campo_input_sev.keyup(function () {
                form_sev.validate();
            });
        };
    ...
    $('#latitudUtmSev').number(true, 3, '.');
    $('#latitudUtmSev').mask('999999.999');

    $('#longitudUtmSev').number(true, 3, '.');
    $('#longitudUtmSev').mask('9999999.999');

    $('#sev_azimut').number(true, 2, '.');
    $('#sev_azimut').mask('999.99');

    $('#sev_abmax').number(true, 2, '.');
    $('#sev_abmax').mask('99.99');

    $('#sev_mnmax').number(true, 2, '.');
    $('#sev_mnmax').mask('99.99');
