...
<script>
    ...
    function item_opcion_editar_sev(id) {
        return '<a class="dropdown-item" href="javascript:item_update_sev(\''+ id +'\');"><i class="la flaticon-edit-1 m--font-brand"></i>&nbsp;Editar</a>';
    }

    function item_opcion_eliminar_sev(id) {
        return '<a class="dropdown-item" href="javascript:item_delete_sev(\''+ id +'\');"><i class="la flaticon-delete-2 m--font-brand"></i>&nbsp;Eliminar</a>';
    }
    ...
    function get_form_sev(id, type) {
        ...
        $.get(url, function(respuesta) {
            $('#modal_content_sev').html(respuesta);
            $('#modal_window_sev').modal("show");
        });
    }
    ...
    var snippet_datatable_list_sev = function () {
        ...
        var exporta_titulo = "SIASBO - GEOFÍSICA - LISTA SEV";
        var exporta_columnas = [':visible :not(.noExport)'];

        var initTable = function() {
            // begin first table
            table_list_sev = $('#lista_sev').DataTable({
                initComplete: function(settings, json) {
                    $('#lista_sev').removeClass('m--hide');
                },
                responsive: true,
                keys: {
                    blurable: false,
                ...
                columnDefs: [
                    {
                        ...
                        render: function(data, type, full, meta) {
                            // var boton = item_opcion_sev(data) + '<span class="dropdown"><a href="#" class="btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill" data-toggle="dropdown" aria-expanded="true"><i class="fa fa-ellipsis-h m--font-brand"></i></a><div class="dropdown-menu dropdown-menu-left">{/literal}{if $privFace.editar == 1}{literal}' + item_opcion_editar_sev(data) + '{/literal}{/if}{literal}{/literal}{if $privFace.eliminar == 1}{literal}' + item_opcion_eliminar_sev(data) + '{/literal}{/if}{literal}</div></span>';
                            // return boton;

                            var boton = item_opcion_sev(data) + '<span class="dropdown"><a href="#" class="btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill" data-toggle="dropdown" aria-expanded="true"><i class="fa fa-ellipsis-h m--font-brand"></i></a><div class="dropdown-menu dropdown-menu-left">';
                            ...
                            if (obj_permiso[0].editar == 1){ //permisoedit
                                botonedit = '{/literal}{if $privFace.editar == 1}{literal}' + item_opcion_editar_sev(data) + '{/literal}{/if}{literal}';
                            }

                            if (obj_permiso[0].eliminar == 1){ //permisodelete
                                botondelete = '{/literal}{if $privFace.eliminar == 1}{literal}' + item_opcion_eliminar_sev(data) + '{/literal}{/if}{literal}';
                            }
        ...
        jQuery.ajax({
            ...
            success: function(data){
                ...
                if (obj_permiso[0].crear == 1){                                
                    $("#btn_nuevo_sev_submit").show();
                }else{
                    $("#btn_nuevo_sev_submit").hide();
                }                 
            },
        });
    }
