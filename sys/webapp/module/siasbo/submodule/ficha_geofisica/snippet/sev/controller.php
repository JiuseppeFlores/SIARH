...
switch($accion) {
    ...
    default:
        ...
        $grill_list = $subObjItem->get_grilla_list_sbm("grilla_sev");
    ...
    case 'itemupdatesql':
        ...
        $item["sev_configId"] = rtrim($optionId, ",");

        $respuesta = $subObjItem->item_update($item, $itemId, "campos_sev", $type, "geofisica");
        /*$ultimoSevId = $respuesta['id'];
        foreach ($item['configuracionId'] as $clave => $valor) {
            $config = array('sevId' => $ultimoSevId, 'configuracionId' => $valor);
            $subObjItem->item_update($config, "", "campos_sev_config", "new", "geofisica_config");
        }*/
        $core->print_json($respuesta);
        break;

    case 'getItemList':
        $respuesta = $subObjItem->get_item_datatable_Rows("geofisica", "grilla_sev", "tipo=1 AND geofisicaId=".$id);
    ...
    case 'itemUpdate':
        ...
        if($type=="update"){
            ...
            $opcionAux = explode(",", $item["sev_configId"]);
            $opcionChecked = array();
            foreach ($opcionAux as $clave => $valor) {
                if (array_key_exists($valor, $cataobj["devconfig"])) {
        ...
        $smarty->assign("subpage", $webm["sev_sc_index"]);
        break;
    ...
    case 'itemupdatesqlCapa':
        $respuesta = $subObjItem->item_update($itemCapa, $itemId, "campos_capa", $type, "geofisica_dev_capa");
        $core->print_json($respuesta);
        break;
    ...
    case 'itemupdatesqlVinculoPozo':
        $respuesta = $subObjItem->item_update($itemVinculoPozo, $itemId, "campos_vinculo_pozo", $type, "geofisica_vinculo_pozo");
        $core->print_json($respuesta);
        break;
