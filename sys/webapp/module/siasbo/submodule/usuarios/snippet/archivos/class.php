...
class Snippet extends Table
{
    ...
    function item_update($rec,$itemId,$que_form,$accion,$item_id,$input_archivo){
        ...
        if( $res["res"]==1){
            ...
            $adjunto = $this->item_adjunto_sbm($input_archivo,$item_id,$res["id"],$item,$tabla,$accion);
        }
        return $res;
    }
    ...
    function get_item($id,$item_id){
        $sql = "select * from ".$this->tabla["archivo"]." as p where p.itemId = '".$id."' and p.item_id = '".$item_id."'";
    ...
    function item_delete($id,$item_id){
        ...
        $campo_id="itemId";
        $where = "item_id='".$item_id."'";
        $res = $this->item_delete_sbm($id,$campo_id,$this->tabla["archivo"],$where);
