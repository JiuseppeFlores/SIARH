...
class Index extends Table {
    ...
    public function get_item_datatable_Rows(){
        ...
        */
         ...
         */
        //$extraWhere = "tipo=1";
        //$extraWhere = "activo=1 and grupoId=21"; //activo=1 and grupoId=21
        $extraWhere = "activo=1";
        $groupBy = "";
        $having = "";
        /**
...
//--------------Permisos---------------------------------
    ...
    //     $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
    //     $sql = "SELECT a.itemId, a.usuario, a.nombre, a.apellido 
    //     FROM vrhr_snir.core_usuario as a  
    //     WHERE a.activo = 1 and a.usuario = 'siasbo'";
    //     $datos = $this->dbm->Execute($sql);
    //     $datos = $datos->GetRows();
    //     return $datos;
    // }

    function obtenerSubmodulos($id){
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT c.itemId 
        ...
        WHERE a.activo = 1 and a.itemId = ".$id; //a.usuario = 'siasbo'
        $datos = $this->dbm->Execute($sql);
        $datos = $datos->GetRows();
        return $datos;
    }

    function permisoBasico($id){
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);

        //$indices = array(272, 273, 275, 276, 277, 278, 279, 281, 282, 283, 284, 286, 287, 288, 498, 499);
        $indices = $this->obtenerSubmodulos($id);
        $numelementos = count($indices);
        
        for ($i=0; $i<=$numelementos-1; $i++){ 
            $datos[] = array(1, 1, 0); //El tipo de permiso para crear, editar y eliminar
        }
        ...
        for ($i=0; $i<$numelementos ; $i++){
            $sqlupdate = "UPDATE vrhr_snir.core_usuario_permisos 
                          SET crear=".$datos[$i][0].", editar=".$datos[$i][1].", eliminar=".$datos[$i][2]." 
                          WHERE usuarioId=".$id." and subModuloId=".$indices[$i][itemId];
            $this->dbm->Execute($sqlupdate);
        }
    ...
    function permisoMedio($id){
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);

        $indices = $this->obtenerSubmodulos($id);
        $numelementos = count($indices);
        for ($i=0; $i<=$numelementos-1; $i++){ 
            $datos[] = array(0, 1, 0); //El tipo de permiso para crear, editar y eliminar
        }
    ...
    function permisoAvanzado($id){
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);

        $indices = $this->obtenerSubmodulos($id);
        $numelementos = count($indices);
        for ($i=0; $i<=$numelementos-1; $i++){ 
            $datos[] = array(1, 1, 1); //El tipo de permiso para crear, editar y eliminar
        }
    ...
    //Permisos de usuario
    function get_permisos($usuario, $tipoUsuario, $itemIdSubmoduloPozo){ //$tiposubmodulo
        // if ($usuario === 'admin'){
        //     return array(array(
        //             'crear' => 1,
        //             'editar' => 1,
        ...
        if ($tipoUsuario == "0" || $tipoUsuario == "1"){
            ...
            // FROM vrhr_snir.core_usuario as a inner join vrhr_snir.core_usuario_permisos as b on a.itemId = b.usuarioId 
            // inner join vrhr_snir.core_submodulo as c on b.subModuloId = c.itemId 
            // WHERE b.usuarioId='".$usuario."' and c.itemId=".$itemIdSubmoduloPozo;
            // $datos = $this->dbm->Execute($sql);
            // $datos = $datos->GetRows();
            // return $datos;
            ...
            // FROM vrhr_snir.core_usuario as a inner join vrhr_snir.core_usuario_permisos as b on a.itemId = b.usuarioId 
            // inner join vrhr_snir.core_submodulo as c on b.subModuloId = c.itemId 
            // WHERE a.tipoUsuario='".$usuario."' and c.itemId='".$itemIdSubmoduloPozo."' limit 1";
            // $datos = $this->dbm->Execute($sql);
            // $datos = $datos->GetRows();
            // return $datos;
            ...
            FROM vrhr_snir.core_usuario as a inner join vrhr_snir.core_usuario_permisos as b on a.itemId = b.usuarioId 
            inner join vrhr_snir.core_submodulo as c on b.subModuloId = c.itemId 
            WHERE a.usuario='".$usuario."' and a.tipoUsuario='".$tipoUsuario."' and c.nombre='".$itemIdSubmoduloPozo."'";
            $datos = $this->dbm->Execute($sql);
            $datos = $datos->GetRows();
            return $datos;
