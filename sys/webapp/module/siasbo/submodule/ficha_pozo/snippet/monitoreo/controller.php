...
switch($accion){
    ...
    case 'getItemListCantidad':
        $respuesta = $subObjItem->get_item_datatable_Rows("monitor", "grilla_cantidad", "pozoId=".$pozoId);
    ...
    case 'itemupdatesqlCantidad':
        $respuesta = $subObjItem->item_update($item, $itemId, "campos_cantidad", $type, "monitor");
        $core->print_json($respuesta);
        break;
    ...
    case 'getItemHidrograma':
        $datosHidrograma = $subObjItem->get_datos_hidrograma($pozoId);
        echo json_encode($datosHidrograma);
        exit;
        break;

    case 'getItemCaudal':
        $datosCaudal = $subObjItem->get_datos_caudal($pozoId);
    ...
    case 'getItemListCalidad':
        $respuesta = $subObjItem->get_item_datatable_Rows("monitor_calidad", "grilla_calidad", "pozoId=".$pozoId);
        foreach ($respuesta['data'] as $clave => $valor) {
    ...
    case 'itemupdatesqlCalidad':
        $respuesta = $subObjItem->item_update($item, $itemId, "campos_calidad", $type, "monitor_calidad");
        $core->print_json($respuesta);
        break;
    ...
    case 'getItemGrillaCalidadCompuesto':
        ...
        $campanias = $subObjItem->get_campanias($pozoId);
    ...
    case 'getItemListCalidadCompuesto':
        $respuesta = $subObjItem->get_item_datatable_Rows("monitor_calidad_dato", "grilla_calidad_compuesto", "calidadId=".$calidadId);
        $core->print_json($respuesta);
        break;
    ...
    case 'itemUpdateCalidadCompuesto':
        ...
        $smarty->assign("id", $id);
        if($type=="update"){
            $item = $subObjItem->get_item($id, "monitor_calidad_dato");
            $smarty->assign("item", $item);
    ...
    case 'itemupdatesqlCalidadCompuesto':
        $respuesta = $subObjItem->item_update($item, $itemId, "campos_calidad_compuesto", $type, "monitor_calidad_dato");
        $core->print_json($respuesta);
        break;

    case 'itemDeleteCalidadCompuesto':
        $respuesta = $subObjItem->item_delete($id, "monitor_calidad_dato");
        $core->print_json($respuesta);
        break;
    ...
    case 'getItemListIsotopico':
        $respuesta = $subObjItem->get_item_datatable_Rows("monitor_isotopico", "grilla_isotopico", "pozoId=".$pozoId);
        foreach ($respuesta['data'] as $clave => $valor) {
            $respuesta['data'][$clave]['fecha_muestreo'] = date_format(date_create($valor['fecha_muestreo']),"d/m/Y");
            $respuesta['data'][$clave]['fecha_analisis'] = date_format(date_create($valor['fecha_analisis']),"d/m/Y");
        }
    ...
    case 'itemUpdateIsotopico':
        ...
        $smarty->assign("id", $id);
        if($type=="update"){
            $item = $subObjItem->get_item($id, "monitor_isotopico");
            $smarty->assign("item", $item);
        }
    ...
    case 'itemupdatesqlIsotopico':
        $respuesta = $subObjItem->item_update($item, $itemId, "campos_isotopico", $type, "monitor_isotopico");
        $core->print_json($respuesta);
        break;

    case 'itemDeleteIsotopico':
        $respuesta = $subObjItem->item_delete($id, "monitor_isotopico");
        $core->print_json($respuesta);
        break;
    ...
    case 'getItemListIsotopicoCompuesto':
        $respuesta = $subObjItem->get_item_datatable_Rows("monitor_isotopico_dato", "grilla_isotopico_compuesto", "isotopicoId=".$isotopicoId);
        $core->print_json($respuesta);
        break;
    ...
    case 'itemUpdateIsotopicoCompuesto':
        ...
        $smarty->assign("id", $id);
        if($type=="update"){
            $item = $subObjItem->get_item($id, "monitor_isotopico_dato");
            $smarty->assign("item", $item);
    ...
    case 'itemupdatesqlIsotopicoCompuesto':
        $respuesta = $subObjItem->item_update($item, $itemId, "campos_isotopico_compuesto", $type, "monitor_isotopico_dato");
        $core->print_json($respuesta);
        break;

    case 'itemDeleteIsotopicoCompuesto':
        $respuesta = $subObjItem->item_delete($id, "monitor_isotopico_dato");
        $core->print_json($respuesta);
        break;
    ...
    case 'getCampanias':
        $respuesta = $subObjItem->get_campanias($pozoId);
        $core->print_json($respuesta);
        break;
