{literal}
<script>
    var snippet_form_cantidad = function () {
        var idficha = '{/literal}{$id}{literal}';
        var type = '{/literal}{$type}{literal}';
        var form_cantidad = $('#form_cantidad');
        var btn_monitor_cantidad_submit = $('#btn_monitor_cantidad_submit');
        var campo_input_cantidad = $('#form_cantidad input');
        ...
        var showResponse = function (responseText, statusText) {
            //alert('{/literal}{$subcontrol}_itemupdatesqlCantidad{literal}');
        ...
        var options = {
            ...
            , data: {
                accion: '{/literal}{$subcontrol}_itemupdatesqlCantidad{literal}'
                ,itemId: idficha
                ,type: type
            }
        };
        ...
        var handle_form_submit = function () {
            form_cantidad.ajaxForm(options);
        };
        ...
        var handle_general_form_submit = function () {
            ...
            btn_monitor_cantidad_submit.click(function (e) {
                ...
                form_cantidad.submit();
            });
        };

        var handle_general_components = function () {
            $('#cant_fecha').datepicker({
                todayHighlight: true,
        ...
        var handle_get_components = function() {
            campo_input_cantidad.keyup(function () {
                form_cantidad.validate();
            });
    ...
    $('#cant_elevacion').number(true, 2, '.');
    $('#cant_elevacion').mask('9999.99');

    $('#cant_nivel_freatico').number(true, 2, '.');
    $('#cant_nivel_freatico').mask('99.99');

    $('#cant_nivel_dinamico').number(true, 2, '.');
    $('#cant_nivel_dinamico').mask('99.99');

    $('#cant_nivel_estatico').number(true, 2, '.');
    $('#cant_nivel_estatico').mask('99.99');

    $('#cant_caudal').number(true, 2, '.');
    $('#cant_caudal').mask('999.99');

    $('#cant_caudal_autorizado').number(true, 2, '.');
    $('#cant_caudal_autorizado').mask('999.99');
    ...
    function permisos_usuario(){
        ...
        jQuery.ajax({
            ...
            success: function(data){
                ...
                //     $("#btn_monitor_cantidad_submit").show();
                // }else{
                //     $("#btn_monitor_cantidad_submit").hide();
                // }   
