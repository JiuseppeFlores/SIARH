{include file="tipo_recuperacion_escalon/form/index.css.tpl"}

<form class="m-form m-form--state m-form--fit m-form--label-align-right" method="POST" action="{$getModule}" id="form_recupera_escalon">
    <input type="hidden" name="item[recuperacionId]" id="tbp_recuperacionId" value="{$recuperacionId}">

    <div class="modal-header">
        <h4 class="modal-title">Datos escalón de recuperación</h4>
    </div>
    ...
    <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary btn-block-custom" id="btn_recupera_escalon_submit"><i class="fa fa-check"></i>&nbsp;{if $type == 'new'}Guardar{else}Actualizar{/if}</button>
    </div>
</form>

{include file="tipo_recuperacion_escalon/form/index.js.tpl"}