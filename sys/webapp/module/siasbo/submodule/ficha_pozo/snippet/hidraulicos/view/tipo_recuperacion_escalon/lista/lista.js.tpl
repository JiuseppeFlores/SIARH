{literal}
<script>
    var table_list_recupera_escalon;

    function item_update_recupera_escalon(id) {
        //var url = '{/literal}{$getModule}{literal}&accion=sev_itemUpdate&id='+id+'&type=update';
        //location = url;
        get_form_recupera_escalon(id, "update");
    }

    function item_delete_recupera_escalon(id) {
        swal({
            ...
            if (result.value) {
                itemDeleteActionRecuperaEscalon(id);
            }
        });
    }

    function item_print_recupera_escalon(id) {
        alert("Imprime el dato:"+id);
    }

    function itemDeleteActionRecuperaEscalon(id) {
        randomnumber=Math.floor(Math.random()*11);
        $.get( "{/literal}{$getModule}{literal}",
            {accion:"{/literal}{$subcontrol}_itemDeleteRecuperaEscalon{literal}", random:randomnumber, id:id},
            function(res){
                if(res.res == 1){
    ...
    function item_opcion_editar_recupera_escalon(id) {
        return '<a class="dropdown-item" href="javascript:item_update_recupera_escalon(\''+ id +'\');" class="m-portlet__nav-link btn m-btn m-btn--hover-warning m-btn--icon m-btn--icon-only m-btn--pill" title="Editar"><i class="la flaticon-edit-1 m--font-brand"></i>&nbsp;Editar</a>';
    }

    function item_opcion_eliminar_recupera_escalon(id) {
        return '<a class="dropdown-item" href="javascript:item_delete_recupera_escalon(\''+ id +'\');" class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" title="Eliminar"><i class="la flaticon-delete-2 m--font-brand"></i>&nbsp;Eliminar</a>';
    }

    function get_form_recupera_escalon(id, type) {
        var url = "{/literal}{$getModule}{literal}&accion={/literal}{$subcontrol}_itemUpdateRecuperaEscalon{literal}&recuperacionId={/literal}{$recuperacionId}{literal}&id="+id+"&type="+type;
    ...
    var snippet_datatable_list_recupera_escalon = function () {
        ...
        var initTable = function() {
            // begin first table
            table_list_recupera_escalon = $('#lista_recupera_escalon').DataTable({

                initComplete: function(settings, json) {
                    $('#lista_recupera_escalon').removeClass('m--hide');
                },
                responsive: true,
                keys: {
                    blurable: false,
                ...
                serverSide: true,
                ajax: {
                    url: '{/literal}{$getModule}{literal}&accion={/literal}{$subcontrol}_getItemListRecuperaEscalon{literal}&recuperacionId={/literal}{$recuperacionId}{literal}',
                    type: 'POST',
                    data: {},
                },
                columns: [
                ...
                columnDefs: [
                    {
                        ...
                        render: function(data, type, full, meta) {
                            ...
                            if (obj_permiso[0].editar == 1){ //permisoedit
                                botonedit = '{/literal}{if $privFace.editar == 1}{literal}' + item_opcion_editar_recupera_escalon(data) + '{/literal}{/if}{literal}';
                            }

                            if (obj_permiso[0].eliminar == 1){ //permisodelete
                                botondelete = '{/literal}{if $privFace.eliminar == 1}{literal}' + item_opcion_eliminar_recupera_escalon(data) + '{/literal}{/if}{literal}';
                            }
    ...
    function get_grafica(id) {
        //alert("{/literal}{$pozoId}{literal}");
        //alert("ID: "+id);
        var url = "{/literal}{$getModule}{literal}&accion={/literal}{$subcontrol}_getItemEscalonRecuperacion{literal}&id="+id;
        ...
        $.get(url, function(respuesta) {
            if (respuesta == 0) {
                ...
                $('#modal_escalon_recu_contenido').html(respuesta);
                $("#modal_escalon_recu").modal("show");
                swal.close();
            }
        });
    }
    ...
    //     RecuperarDatosEscalonRecuperacion();
    ...
    // function AbrirModal(){
    //     $('#modalgraficarecuperacion').modal('show');
    // }
    
    // function CerrarModal(){
    //     $('#modalgraficarecuperacion').modal('hide');
    // }
...
//----------------Permisos--------------------------------------------
    ...
    function permisos_usuario(){
        ...
        jQuery.ajax({
            ...
            success: function(data){
                ...
                //     $("#btn_nuevo_recupera_escalon_submit").show();
                // }else{
                //     $("#btn_nuevo_recupera_escalon_submit").hide();
                // }       
    ...
    jQuery(document).ready(function() {
        permisos_usuario();
        snippet_datatable_list_recupera_escalon.init();
        //RecuperarDatosEscalonRecuperacion();
    });
