<?php
class Snippet extends Table
{
    var $item_form;
    function __construct()
    {
        /**
         * Inicializamos todas las librerias y variables para el submodulo
         */
        $this->submodule_init_sbm();

    }
    /**
     * Function que actualiza la informacion de datos de un registro
     * $respuesta = $subObjItem->item_update($item,$itemId,0);
     * $rec = arreglo de datos que llega del formulario
     * $itemId= el id del reguistro que quiero actualizar
     * $que_form = El formulario dentro que quiero actualizar, "Es el mismo nombre del grupo de campos que quiero validar"
     * $accion = new, update ,  solo existen 2 acciones
     *
     **/
    function item_update($rec,$itemId,$que_form,$accion){
        global $privFace;
        /**
         * preprocesamos los datos
         */
        $respuesta_procesa = $this->procesa_datos($que_form,$rec,$accion);
        /**
         * Guardo los datos ya procesados
         */
        $campo_id="itemId";
        $res = $this->item_update_sbm($itemId,$respuesta_procesa,$this->tabla["pozo"],$accion,$campo_id);

        $res["accion"] = $accion;

        return $res;
    }

    /**
     * Funcion que valida los campos a ser guardados
     **/
    function procesa_datos($que_form,$rec,$accion="new"){
        $dato_resultado = array();
        switch($que_form){
            case 'campos_perforacion':
                /**
                 * $rec = es el arreglo de datos que proviene del formulario, desde el frontend
                 * $$this->item_form = es el arreglo de configuraciòn de datos que se procesaran
                 * $accion = new, update
                 */
                $dato_resultado = $this->procesa_campos_sbm($rec,$this->campos[$que_form],$accion);
                /**
                 * Si es nuevo el registro, haré que el ID sea igual a la gestion_id
                 */

                /**
                 * Procesos Adicionales al momento de guardar los datos
                 */

            // case 'campos_perforacion_excavado':
            //     $dato_resultado = $this->procesa_campos_sbm($rec,$this->campos[$que_form],$accion);

                break;
            case 'otros_formularios':
                break;
        }
        return $dato_resultado;
    }

    // function get_perforacion($Id){
    //     $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
    //     $sql = "SELECT
    //     itemId,
    //     perforacion_fecha,
    //     perforacion_pozoId,
    //     perforacion_tipoId,
    //     perforacion_metodoId,
    //     perforacion_profundidad,
    //     perforacion_diametro,
    //     perforacion_diametro_final,
    //     perforacion_observaciones 
    //     FROM item_pozo
    //     WHERE itemId=".$Id;
    //     //".$this->tabla["item_pozo"]."

    //     $datos = $this->dbm->Execute($sql);
    //     $datos = $datos->GetRows();

    //     return $datos;
    // }

    // function get_tipopozo(){
    //     $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
    //     $sql = "SELECT
    //     itemId,
    //     nombre 
    //     FROM catalogo_pozo_perforacion 
    //     WHERE activo=1";
    //     //".$this->tabla["item_pozo"]."

    //     $datos = $this->dbm->Execute($sql);
    //     $datos = $datos->GetRows();

    //     return $datos;
    // }

    // function get_tipoperforacion(){
    //     $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
    //     $sql = "SELECT
    //     itemId,
    //     nombre 
    //     FROM catalogo_pozo_perforacion_tipo  
    //     WHERE activo=1";
    //     //".$this->tabla["item_pozo"]."

    //     $datos = $this->dbm->Execute($sql);
    //     $datos = $datos->GetRows();

    //     return $datos;
    // }

    // function get_metodoperforacion(){
    //     $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
    //     $sql = "SELECT
    //     itemId,
    //     nombre 
    //     FROM catalogo_pozo_perforacion_metodo 
    //     WHERE activo=1";
    //     //".$this->tabla["item_pozo"]."

    //     $datos = $this->dbm->Execute($sql);
    //     $datos = $datos->GetRows();

    //     return $datos;
    // }
}