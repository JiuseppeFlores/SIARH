{literal}
<script>
    var table_list_fuente;

    function item_update_fuente(id) {
        //var url = '{/literal}{$getModule}{literal}&accion=sev_itemUpdate&id='+id+'&type=update';
        //location = url;
        get_form_fuente(id, "update");
    }

    function item_delete_fuente(id) {
        swal({
    ...
    function item_opcion_editar_fuente(id) {
        return '<a class="dropdown-item" href="javascript:item_update_fuente(\''+ id +'\');" class="m-portlet__nav-link btn m-btn m-btn--hover-warning m-btn--icon m-btn--icon-only m-btn--pill" title="Editar"><i class="la flaticon-edit-1 m--font-brand"></i>&nbsp;Editar</a>';
    }

    function item_opcion_eliminar_fuente(id) {        
        return '<a class="dropdown-item" href="javascript:item_delete_fuente(\''+ id +'\');" class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" title="Eliminar"><i class="la flaticon-delete-2 m--font-brand"></i>&nbsp;Eliminar</a>';
    }

    function get_form_fuente(id, type) {
        ...
        $.get(url, function(respuesta) {
            $('#modal_content_fuente').html(respuesta);
            swal.close();
            $('#modal_window_fuente').modal("show");
        });
    }
    
    var snippet_datatable_list_fuente = function () {
        ...
        var initTable = function() {
            // begin first table
            table_list_fuente = $('#lista_fuente').DataTable({

                initComplete: function(settings, json) {
                    $('#lista_fuente').removeClass('m--hide');
                },
                responsive: true,
                keys: {
                    blurable: false,
                ...
                columnDefs: [
                    {
                        ...
                        render: function(data, type, full, meta) {
                            ...
                            if (obj_permiso[0].editar == 1){ //permisoedit
                                botonedit = '{/literal}{if $privFace.editar == 1}{literal}' + item_opcion_editar_fuente(data) + '{/literal}{/if}{literal}';
                            }

                            if (obj_permiso[0].eliminar == 1){ //permisodelete
                                botondelete = '{/literal}{if $privFace.eliminar == 1}{literal}' + item_opcion_eliminar_fuente(data) + '{/literal}{/if}{literal}';
                            }
    ...
    function RecuperarDatosfuente(){
        var url = "{/literal}{$getModule}{literal}&accion={/literal}{$subcontrol}_getItemFuente{literal}&id={/literal}{$id}{literal}";
        console.log("RecuperarDatosfuente url: "+url);
        swal({
            title: 'Cargando tab!',
            text: 'Procesando datos',
            imageUrl: 'images/loading/loading05.gif',
    ...
    jQuery(document).ready(function() {
        permisos_usuario();
        snippet_datatable_list_fuente.init();
        // RecuperarDatosFuente();
        //DibujarColumnaseguimiento();
    });
