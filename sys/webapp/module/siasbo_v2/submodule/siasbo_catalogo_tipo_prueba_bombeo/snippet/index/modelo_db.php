...
class ModeloBD {
	...
	public function listarRegistros() {
		try {
            ...
            $resultado = $conexion->query('SELECT itemId, nombre, estado from tipo_prueba_bombeo');
	...
	public function crearRegistro($datos, $accion) {
		try {
			...
			$transaccion = $conexion->prepare("INSERT INTO tipo_prueba_bombeo (nombre, estado) VALUES (:nombre, :estado)");
	...
	public function actualizarRegistro($datos, $accion, $id) {
		try {
			$conexion = $this->abrirConexion();
			$conexion->beginTransaction();
			$transaccion = $conexion->prepare("UPDATE tipo_prueba_bombeo SET nombre=:nombre, estado=:estado WHERE itemId=:itemId LIMIT 1");
			$transaccion->bindParam(':nombre', $datos['nombre']);
			$transaccion->bindParam(':estado', $datos['estado']);
	...
	public function eliminarRegistro($accion, $id) {
		try {
			$conexion = $this->abrirConexion();
			$conexion->beginTransaction();
			$transaccion = $conexion->prepare("DELETE FROM tipo_prueba_bombeo WHERE itemId=:itemId LIMIT 1");
			$transaccion->bindParam(':itemId', $id);
			$transaccion->execute();
			$conexion->commit();
            $cantidadRegistros = $transaccion->rowCount();
	...
	public function desactivarRegistro($accion, $id) {
		try {
			$conexion = $this->abrirConexion();
			$conexion->beginTransaction();
			$transaccion = $conexion->prepare("UPDATE tipo_prueba_bombeo SET estado='INACTIVO' WHERE itemId=:itemId LIMIT 1");
			$transaccion->bindParam(':itemId', $id);
			$transaccion->execute();
			$conexion->commit();
            $cantidadRegistros = $transaccion->rowCount();
