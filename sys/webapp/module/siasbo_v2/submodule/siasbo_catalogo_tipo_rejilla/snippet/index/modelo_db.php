...
class ModeloBD {
	...
	public function listarRegistros() {
		try {
            ...
            $resultado = $conexion->query('SELECT itemId, nombre, estado from tipo_rejilla');
	...
	public function crearRegistro($datos, $accion) {
		try {
			...
			$transaccion = $conexion->prepare("INSERT INTO tipo_rejilla (nombre, estado) VALUES (:nombre, :estado)");
	...
	public function actualizarRegistro($datos, $accion, $id) {
		try {
			$conexion = $this->abrirConexion();
			$conexion->beginTransaction();
			$transaccion = $conexion->prepare("UPDATE tipo_rejilla SET nombre=:nombre, estado=:estado WHERE itemId=:itemId LIMIT 1");
			$transaccion->bindParam(':nombre', $datos['nombre']);
			$transaccion->bindParam(':estado', $datos['estado']);
	...
	public function eliminarRegistro($accion, $id) {
		try {
			$conexion = $this->abrirConexion();
			$conexion->beginTransaction();
			$transaccion = $conexion->prepare("DELETE FROM tipo_rejilla WHERE itemId=:itemId LIMIT 1");
			$transaccion->bindParam(':itemId', $id);
			$transaccion->execute();
			$conexion->commit();
            $cantidadRegistros = $transaccion->rowCount();
	...
	public function desactivarRegistro($accion, $id) {
		try {
			$conexion = $this->abrirConexion();
			$conexion->beginTransaction();
			$transaccion = $conexion->prepare("UPDATE tipo_rejilla SET estado='INACTIVO' WHERE itemId=:itemId LIMIT 1");
			$transaccion->bindParam(':itemId', $id);
			$transaccion->execute();
			$conexion->commit();
            $cantidadRegistros = $transaccion->rowCount();
